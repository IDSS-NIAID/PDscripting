  ...
  tibble(name = map_chr(colDescriptions, ~ .x$ColumnName),
         type = map_chr(colDescriptions, ~
                          case_when(.x$ID       == "ID"      ~ 'c', # IDs are large negative integers
                                    ...
                                    .x$DataType == "Int"     ~ 'i',
                                    TRUE                     ~ '?')),
         new_name = case_when(grepl("Abundances Normalized", name) ~
                                str_replace(name, ' ', '.') |> # convert to "Abundances.Normalized <sample Id>.Sample"
                              ...
                              grepl("Found in Sample",       name) ~
                                str_replace(name, ' ', '.') |> # convert to "Found.in.Samples <sample Id>.Sample"
                                str_replace(' ', '.') |>
                                str_replace(' ', '_') |>       # convert to "Found.in.Samples_<sample Id>.Sample"
                                str_replace_all(' ', '.'),     # convert to "Found.in.Samples_<sample.Id>.Sample"

                              grepl("Abundance Ratio",       name) ~
                                str_replace(name, ' ', '.') |> # convert to "Abundance.Ratio* <sample Id>  <sample Id>"
                              ...
                              grepl('Abundances Grouped',    name) ~
                                str_replace(name, ' ', '.') |> # convert to "Abundances.Grouped* <sample Id>"
